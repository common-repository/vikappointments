 ...
 * The message will be displayed only once within the dashboard of VikAppointments.
 *
 * @since 1.2.13
 */
add_action('vikappointments_before_display_vikappointments', array('VikAppointmentsInstaller', 'showBreakingChanges'));
...
add_action('plugins_loaded', array('VikAppointmentsBuilder', 'setupOverridesManager'));
...
if (JFactory::getApplication()->isAdmin() && !wp_doing_ajax())
{
	...
	// add new buttons
	add_filter('mce_buttons', ['VikAppointmentsTinyMCE', 'addShortcodesButton']);

	// load the button handlers
	add_filter('mce_external_plugins', ['VikAppointmentsTinyMCE', 'registerShortcodesScript']);
}
...
add_action('init', ['VikAppointmentsGutenberg', 'registerShortcodesScript']);
 ...
 * 
 * Short-circuit the usage of wptexturize for the whole front-end.
 * 
 * @param  bool  $run  Whether to short-circuit wptexturize().
 *
 * @since 1.2.13
 */
add_filter('run_wptexturize', function($run)
{
	return is_admin() ? $run : false;
}, PHP_INT_MAX);
