(function(wp) {
	 ...
	 */
	registerBlockType('vikappointments/gutenberg-shortcodes', {
		/**
		 * This is the block display title, which can be translated with `i18n` functions.
		 * The block inserter will show this name.
		 */
		title: __('VikAppointments Shortcode', 'vikappointments'),

		/**
		 * This is the block description, which is displayed within the right sidebar.
		 */
		description: __('Add a shortcode configured through VikAppointments.', 'vikappointments'),
		...
		edit: (props) => {
			...
			// iterate vikappointments shortcodes to build select options
			let options = [];
			...
			// insert empty option
			options.push({
				label: __('- pick a shortcode -', 'vikappointments'),
				value: '',
			});

			// insert an option to support the creation of new shortcodes
			options.push({
				label: __('- create a new shortcode -', 'vikappointments'),
				value: 'new',
			});
			...
			for (let group in VIKAPPOINTMENTS_SHORTCODES_BLOCK.shortcodes) {
				let groups = VIKAPPOINTMENTS_SHORTCODES_BLOCK.shortcodes[group];
				...
				for (let i = 0; i < groups.length; i++) {
					...
					if (togglerChecked || !post_id || post_id === wp.data.select('core/editor').getCurrentPostId() || data.shortcode == currentShortcode) {
						...
						if (post_id && data.shortcode != currentShortcode) {
							assigneeField = el(
								'a',
								{
									...
									onClick: () => {
										alert(__('This shortcode is already used by a different post. If you select this shortcode, it will be detached from the existing post and assigned to this new post.', 'vikappointments'));
									},
								},
								el(
									'span',
									...
									__('Post #', 'vikappointments') + post_id
								)
							);
						} else {
							// safe shortcode
							assigneeField = el('span', {}, (post_id ? __('Post #', 'vikappointments') + post_id : '--'));
						}
						...
						shortcodes_boxes.push(el(
							'div',
							{
								className: 'vikappointments-shortcode-info-box' + (toggled ? ' toggled' : ''),
							},
							// create child elements
							el(
								'div',
								{
									className: 'vap-sh-info-control'
								},
								[
									el('label', {}, __('Type:', 'vikappointments')),
									el('span', {}, group),
								]
							),
							el(
								...
								[
									el('label', {}, __('Name:', 'vikappointments')),
									el('span', {}, data.name),
								]
							),
							el(
								...
								[
									el('label', {}, __('Created on:', 'vikappointments')),
							...
							el(
								...
								[
									el('label', {}, __('Assignee:', 'vikappointments')),
									assigneeField
								]
							),
							el(
								...
								el(
									...
									{
										label: __('Shortcode:', 'vikappointments'),
										value: data.shortcode,
										readonly: true,
									}
								)
			...
			navBar.push(
				el(
					...
					el(
						...
						{
							label: __('See all', 'vikappointments'),
							checked: togglerChecked,
							help: togglerChecked ? __('Display all the existing shortcodes.', 'vikappointments') : __('Toggle to display also the shortcodes that are already assigned to a post.', 'vikappointments'),
							onChange: (toggler) => {
			...
			if (props.attributes.shortcode === 'new') {
				...
				if (!newShortcodeRegistry) {
					...
					newShortcodeRegistry = new VAPBlockPropertiesDecorator({lang: '*'}, props);
				} else {
				...
				newItemPanelFields.push(
					el(
						...
						{
							label: __('Page Type', 'vikappointments'),
							help: __('Choose the type of the page that should be displayed in the front-end.', 'vikappointments'),
							value: newShortcodeRegistry.attributes.type,
							options: [
								{
									label: __('Select an option', 'vikappointments'),
									value: '',
								}
							].concat(VIKAPPOINTMENTS_SHORTCODES_BLOCK.views.map((page) => {
								return {
									label: page.name,
									value: page.type,
								};
				...
				newItemPanelFields.push(
					el(
						wp.components.SelectControl,
						{
							label: __('Language', 'vikappointments'),
							help: __('Choose whether the shortcode should be available for a specific language only.', 'vikappointments'),
							value: newShortcodeRegistry.attributes.lang,
							options: [
								{
									label: __('All', 'vikappointments'),
									value: '*',
								}
							].concat(VIKAPPOINTMENTS_SHORTCODES_BLOCK.languages.map((lang) => {
								return {
									label: lang.name,
									value: lang.tag,
								};
				...
				let selectedView = VIKAPPOINTMENTS_SHORTCODES_BLOCK.views.filter(view => view.type === newShortcodeRegistry.attributes.type).shift();
				...
				if (selectedView) {
					selectedView.fields.forEach((field) => {
						...
						let fieldControl = window.vapCreateControlElement(field, newShortcodeRegistry);
				...
				navBar.push(
					el(
						wp.components.PanelBody,
						{
							title: __('New Item', 'vikappointments'),
							initialOpen: true,
						},
						newItemPanelFields
					)
				...
				newItemPanelFields.push(
					el(
						...
						{
							text: __('Create Shortcode', 'vikappointments'),
							isPrimary: true,
							onClick: (event) => {
								...
								if (parentId) {
									request.parent_id = (
										...
										Object.values(VIKAPPOINTMENTS_SHORTCODES_BLOCK.shortcodes).flat().filter(shortcode => shortcode.post_id == parentId).shift()
								...
								new Promise((resolve, reject) => {
									...
									if (!request.name) {
										reject(__('You need to specify a title for this post first.', 'vikappointments'));
										return;
									}

									// make request to save the shortcode
									jQuery.ajax({
										url: VIKAPPOINTMENTS_SHORTCODES_BLOCK.ajaxurl,
										type: 'post',
										data: request,
									}).done((shortcode) => {
										resolve(shortcode);
									...
									if (VIKAPPOINTMENTS_SHORTCODES_BLOCK.shortcodes.custom === undefined) {
										VIKAPPOINTMENTS_SHORTCODES_BLOCK.shortcodes.custom = [];
									}

									// append the newly created shortcode to the custom slot
									VIKAPPOINTMENTS_SHORTCODES_BLOCK.shortcodes.custom.push(shortcode);

									// update the selected shortcode
									props.setAttributes({shortcode: shortcode.shortcode});
								}).catch((error) => {
									// something went wrong, alert the user
									alert(error || __('An error has occurred', 'vikappointments'));
			...
			if (props.attributes.shortcode === 'new') {
				renderer = el(
					...
					__('You can create a new shortcode from the block settings under the right sidebar.', 'vikappointments')
				);
			} else {
				renderer = el(
					wp.serverSideRender,
					{
						block: 'vikappointments/gutenberg-shortcodes',
			...
			return [
				...
				el(
					...
					{
						className: 'vap-shortcode-admin-wrapper',
					},
					// <div> contains select
					[
						el(
	 ...
	 */
	const VAPBlockPropertiesDecorator = class VAPBlockPropertiesDecorator {
