...
$serviceId  = (int) $params->get('id_service');
$employeeId = (int) $params->get('id_employee');
			...
			<div class="opb-step-body">
				...
				else
				{
					...
					<div class="opb-search-field service-field">
						...
						<?php if ($serviceId): ?>
							<label><strong><?php echo $selectedService->name ?? ''; ?></strong></label>
						<?php else: ?>
							<label for="vap-opb-search-service<?php echo $module_id; ?>"><?php echo JText::translate('VAP_OPB_SERVICE_LABEL'); ?></label>
						<?php endif; ?>

						<select id="vap-opb-search-service<?php echo $module_id; ?>" data-id="<?php echo $module_id; ?>" style="<?php echo $serviceId ? 'display: none;' : ''; ?>">
							<?php
							foreach ($groups as $group)
							{
								...
								foreach ($group->services as $s)
								{
									...
									<option
										...
										<?php echo $s->id == ($selectedService->id ?? null) ? 'selected="selected"' : ''; ?>
										data-random="<?php echo (int) $s->random_emp; ?>"
										data-first-date="<?php echo htmlspecialchars((string) $s->first_date); ?>"
										data-last-date="<?php echo htmlspecialchars((string) $s->last_date); ?>"
					...
					<div class="opb-search-field employee-field" style="<?php echo $employees && ($selectedService->choose_emp ?? 0) ? '' : 'display: none;'; ?>">

						<?php if ($employeeId): ?>
							<label><strong><?php echo $employees[0]->nickname ?? null; ?></strong></label>
						<?php else: ?>
							<label for="vap-opb-search-employee<?php echo $module_id; ?>"><?php echo JText::translate('VAP_OPB_EMPLOYEE_LABEL'); ?></label>
						<?php endif; ?>

						<select id="vap-opb-search-employee<?php echo $module_id; ?>" style="<?php echo $employeeId ? 'display: none;' : ''; ?>">
							<?php if ($selectedService->random_emp ?? false): ?>
								<option></option>
							<?php endif; ?>
							...
							<?php foreach ($employees as $e): ?>
								<option
									...
									<?php echo $e->id == $employeeId ? 'selected="selected"' : ''; ?>
								><?php echo $e->nickname; ?></option>
							<?php endforeach; ?>
						</select>
					...
					<div class="opb-search-field people-field" style="<?php echo ($selectedService->max_capacity ?? 1) > 1 && ($selectedService->max_per_res ?? 1) > 1 ? '' : 'display: none;'; ?>">
						...
						<select id="vap-opb-search-people<?php echo $module_id; ?>">
							<?php
							for ($i = ($selectedService->min_per_res ?? 1); $i <= ($selectedService->max_per_res ?? 1); $i++)
							{
								?>
	...
	(function(w) {
		if (typeof ONEPAGE_BOOKING_CONFIG === 'undefined')
		{
			w['ONEPAGE_BOOKING_CONFIG'] = {
				...
				/**
				 * The preferred timezone selected by the user.
				 * In case the multi-timezone feature is disabled, this value will be 
				 * equal to the default timezone configured on the website.
				 * 
				 * @var string
				 * @since 1.0.8
				 */
				timezone: '<?php echo VikAppointments::getUserTimezone()->getName(); ?>',
			};
		}
	})(window);
</script>
