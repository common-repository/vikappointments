...
class VikAppointmentsOnepageBookingHelper
{
	 ...
	 * 
	 * @param   JRegistry  $params  The module parameters (@since 1.1).
	 * 
	 * @return 	array
	 */
	public static function getServices($params)
	{
		...
		/**
		 * Take only the services assigned to the selected employee.
		 * 
		 * @since 1.1
		 */
		if ($employeeId = $params->get('id_employee'))
		{
			$q->innerjoin($dbo->qn('#__vikappointments_ser_emp_assoc', 'a') . ' ON ' . $dbo->qn('a.id_service') . ' = ' . $dbo->qn('s.id'));
			$q->where($dbo->qn('a.id_employee') . ' = ' . (int) $employeeId);
		}

		/**
		 * Take only the selected service.
		 * 
		 * @since 1.1
		 */
		if ($serviceId = $params->get('id_service'))
		{
			$q->where($dbo->qn('s.id') . ' = ' . (int) $serviceId);
		}
	 ...
	 * @return 	array
	 */
	public static function getEmployees($id_service, $params)
	{
		if ($id_service <= 0)
		{
			return [];
		...
		{	
			/**
			 * Take only the selected employee.
			 * 
			 * @since 1.1
			 */
			if ($employeeId = $params->get('id_employee'))
			{
				$employees = array_values(array_filter($employees, function($employee) use ($employeeId) {
