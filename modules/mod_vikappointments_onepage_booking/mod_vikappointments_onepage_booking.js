(function($, w) {
	...
	const getUserTimezone = (container) => {
		 ...
		 * 
		 * @since 1.0.8  Use the default system timezone.
		 */
		return ONEPAGE_BOOKING_CONFIG.timezone;
	}
	...
	const bookNowAction = (time, skipOptions) => {
		...
		try
		{
			...
			options = vapGetSelectedOptions(searchData.people);
		}
		catch (error)
		{
	...
	$(function() {
		...
		onInstanceReady(() => {
			...
			return typeof VAPCurrency !== 'undefined' && VAPCurrency.getInstance().symbol;
		}).then(() => {
			currency = VAPCurrency.getInstance();
		});
		...
		$('.vap-opb-container .opb-search-field.service-field select').on('change', function() {
			...
			// preserve the value currently set
			let empId = empBox.find('select').select2('val');
			...
			opbXhr = UIAjax.do(
				...
				(resp) => {
					...
					if (resp && resp.length) {
						...
						// re-select the previous employee is supported by this new service too
						if (empBox.find('select option[value="' + empId + '"]').length) {
							empBox.find('select').select2('val', empId);
						}
					...
					if (data.timeline) {
						...
						data.timeline.forEach((shift) => {
							...
							shift.forEach((slot) => {
								 ...
								 */
								let checkinDate = new Date(slot.utc + 'Z');
								 ...
								 * @since 1.0.6  Append Zulu ID to treat the datetime as UTC.
								 */
								let checkoutDate = new Date(slot.utc_checkout + 'Z');
								 ...
								 * @since 1.0.6
								 * 
								 * Register the time according to the timezone of the employee/system.
								 * 
								 * @since 1.0.8
								 */
								let bookNowButton = item.find('.book-now')
									.attr('data-hour', new Date(slot.checkin).getHours())
									.attr('data-min', new Date(slot.checkin).getMinutes())
