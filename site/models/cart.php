...
class VikAppointmentsModelCart extends JModelVAP
{
	...
	public function addItem($data)
	{
		...
		foreach ($data['options'] as $opt)
		{
			 ...
			 * @since 1.7.5
			 */
			if ($option->maxqpeople == 1 && $option->shared)
			{
				$groupId = $option->id_group ?: 0;
				$sharedOptionsCountLookup[$groupId] = ($sharedOptionsCountLookup[$groupId] ?? 0) + $option->quantity;

				// in case the total count of selected units (for this group) exceeds the maximum capacity,
				// abort and return an error message to the customer
				if ($sharedOptionsCountLookup[$groupId] > $option->maxq)
				{
					// we don't need to translate this message because it can be obtained only
					// by trying to hijack the function used to validate the options
					$this->setError('Maximum quantity exceeded for the selected options!');
					return false;
				}
			}
