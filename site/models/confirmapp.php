...
class VikAppointmentsModelConfirmapp extends JModelVAP
{
	...
	public function save($data)
	{
		...
		if ($cart->getCartLength() > 1)
		{
			...
			// unset payment charge to avoid increasing the total cost
			// also for the children appointments
			$order['payment_charge'] = 0;
			$order['payment_tax']    = 0;
		}
		else
		...
		/**
		 * In case all the appointments registration failed, abort the saving process.
		 * 
		 * @since 1.7.6
		 */
		if (!$ordnum)
		{
			// propagate error
			$this->setError($appModel->getError());
			return false;
		}
