...
class VikAppointmentsModelEmployeeslist extends JModelVAP
{
		...
		if (!$this->pagination)
		{
			...
			foreach ($filters as $k => $v)
			{
				...
				if ($v !== null && $v !== '')
				{
					...
					{
						/**
						 * Added support to multi-selection filters.
						 * 
						 * @since 1.7.6
						 */
						foreach ($v as $iv)
						{
							$this->pagination->setAdditionalUrlParam("filters[$k][]", $iv);
						}
					}
					else
					{
	...
	protected function extendWithCustomFilters(&$q, array $filters, $dbo)
	{
		...
		foreach ($fields as $field)
		{
			...
			{
				/**
				 * Search in JSON array to support multi-selection fields.
				 * 
				 * @since 1.7.6
				 */
				foreach ($filters[$key] as $v)
				{
					$q->where($dbo->qn('e.' . $key) . ' REGEXP ' . $dbo->q('(\[|,)"?' . $v . '"?(,|\])'));
				}
			}
			else
			{
