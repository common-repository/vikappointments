...
class VAPEmployeeAuth
{
	...
	/**
	 * Helper method used to obtain all the required custom fields that haven't been filled
	 * in by the currently logged-in employee.
	 * 
	 * @param   array|null  $fields  The list of custom fields to compare against.
	 * 
	 * @return  string[]    A list of missing custom fields (form field names).
	 * 
	 * @since   1.7.6
	 */
	public function getMissingRequiredFields($fields = null)
	{
		if ($fields === null)
		{
			// auto-load the custom fields
		...
		foreach ($fields as $field)
		{
			...
			{
				// the field is not mandatory, go ahead
				continue;
			}

			// get currently specified value
			$value = $this->employee['field_' . $field['formname']] ?? null;

			if ($value === null || $value === '')
			{
				// inject form name within the list of missing fields
				$missing[] = $field['formname'];
			}
		}
