...
/**
 * Widget used to draw trend count of the options.
 *
 * @since 1.7.5
 */
class VAPStatisticsWidgetOptionsRevenueCount extends VAPStatisticsWidget
{
	/**
	...
	public function getForm()
	{
		$form = array(
			...
			'valuetype' => array(
				...
				'options'  => array(
					...
					'count' => JText::translate('VAPREPORTSVALUETYPEOPT5'),
				),
			),
			...
			/**
			 * A list of options to track.
			 *
			 * @var select
			 */
			'options' => array(
				'type'     => 'select',
				'label'    => JText::translate('VAPMENUOPTIONS'),
				'multiple' => true,
				'options'  => JHtml::fetch('vaphtml.admin.options'),
			),
		);
		...
		if (!$this->hasFinanceAccess)
		{
			...
			// display by default the total number of options
	...
	public function getData()
	{
		...
		$filters['options']    = array_values(array_filter($this->getOption('options', [])));
		...
		if (!$this->hasFinanceAccess)
		{
			// always display the total number of options in case
		...
		// fetch options trend
		$data = VAPStatisticsHelperOptions::getRevenueCount($filters['datefrom'], $filters['dateto'], $filters['valuetype'], $filters['variations'], $filters['options']);
