...
/**
 * Widget used to fetch the most purchased options.
 *
 * @since 1.7.5
 */
class VAPStatisticsWidgetOptionsRevenueItems extends VAPStatisticsWidget
{
	/**
	...
	{
		return empty($group) || $group == '*' || $group == 'dashboard' || $group == 'options';
	}
	...
	{
		return $user->authorise('core.access.analytics.options', 'com_vikappointments');
	}
	...
	public function getForm()
	{
		return array(
			...
			/**
			 * Whether the variations of the same option should be
			...
			'variations' => array(
				'type'  => 'checkbox',
				'label' => JText::translate('VAP_STATS_WIDGET_OPTIONS_REVENUE_ITEMS_VAR_FIELD'),
				'help'  => JText::translate('VAP_STATS_WIDGET_OPTIONS_REVENUE_ITEMS_VAR_FIELD_HELP'),
			),
			...
			'hideunsold' => array(
				'type'  => 'checkbox',
				'label' => JText::translate('VAP_STATS_WIDGET_OPTIONS_REVENUE_ITEMS_HIDE_UNSOLD_FIELD'),
				'help'  => JText::translate('VAP_STATS_WIDGET_OPTIONS_REVENUE_ITEMS_HIDE_UNSOLD_FIELD_HELP'),
			),
	...
	public function export($rule = null)
	{
		...
		else
		{
			...
			$head = [
				// option
				JText::translate('VAPOPTIONFIELDSETTITLE1'),
				// quantity
	...
	public function getOrdering()
	{
		...
		$ordering = strtolower($this->getOption('ordering', 'option.asc'));
	...
	protected function fetchData()
	{
		...
		$filters['variations'] = $this->getOption('variations', false);
		$filters['hideunsold'] = $this->getOption('hideunsold', false);
		...
		// import options helper
		VAPLoader::import('libraries.statistics.helpers.options');
		// fetch option details
		$data['data'] = VAPStatisticsHelperOptions::getRevenueData($filters['datefrom'], $filters['dateto'], $filters['variations']);

		if ($filters['hideunsold'])
		{
			$data['data'] = array_filter($data['data'], function($option) {
				return !empty($option['count']);
			});
		}

		$data['footer'] = [];

		foreach ($data['data'] as $option => $totals)
		{
		...
		if ($ordering['column'] == 'option')
		{
			if ($ordering['direction'] == 'desc')
			{
				// reverse ordering in case of descending direction
		...
		else
		{
			...
			{
				$a = $a[$ordering['column']] ?? 0;
				$b = $b[$ordering['column']] ?? 0;
