...
class VAPAvailabilityImplementor extends VAPAvailabilitySearch
{
	...
	public function getReservations($date, $end = null, $id = 0)
	{
		...
		if ($id_emp)
		{
			...
			/**
			 * Group by closure too in order to keep pulling the closures even if the
			 * the latter are already intersecting some valid appointments.
			 * 
			 * @since 1.7.6
			 */
			$q->group($dbo->qn('r.closure'));
		}
		else
		{
	...
	public function isServicePublished($date)
	{
		...
		/**
		 * The access level is now ignored if we are in the back-end.
		 * 
		 * @since 1.7.6
		 */
		if (!$this->isAdmin() && $levels && !in_array($override->level, $levels))
		{
