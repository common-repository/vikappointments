...
/**
 * Employee area subscription order controller.
 *
 * @since 1.6
 */
class VikAppointmentsControllerEmpSubscrOrder extends VAPEmployeeAreaController
{
	/**
	...
	public function activatetrial()
	{
		...
		// make sure the TRIAL subscription exists
		$trial = $model->getTrial();

		if ($trial === false)
		{
			$this->setRedirect('index.php?option=com_vikappointments&view=empsubscr');
			return false;
		}
		...
		// set the TRIAL subscription into the cart
		$model->setSubscription($trial['id']);
		
		// dispatch saving process
		$this->saveorder();
	}
	...
	public function saveorder()
	{
		 ...
		 * Check token in both POST and GET because the trial activation
		 * is always reached via URL.
		 *
		 * @since 1.7
		 */
		...
		$args['billing'] = $input->get('billing', array(), 'array');
	...
	public function notifypayment()
	{
		...
		$order = VAPOrderFactory::getEmployeeSubscription($oid, null, array('sid' => $sid));
		...
		$return_url = "index.php?option=com_vikappointments&view=empsubscrorder&id={$oid}";
		$error_url  = "index.php?option=com_vikappointments&view=empsubscrorder&id={$oid}";
		...
		$notify_url = "index.php?option=com_vikappointments&task=empsubscrorder.notifypayment&ordnum={$oid}&ordkey={$sid}";
