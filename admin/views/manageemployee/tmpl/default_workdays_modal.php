...
JText::script('VAPMANAGEWDCLONEFROM');
...
<div class="inspector-form" id="inspector-wd-form">
	...
	<div class="inspector-fieldset">
		...
		<?php echo $vik->openControl(''); ?>
			<div class="btn-group input-append" id="wd-add-btn-group">
				<button type="button" class="btn" id="wd-add-time"><?php echo JText::translate('VAPMANAGEWD8'); ?></button>
				<button type="button" class="btn" id="wd-add-options" style="padding-left: 8px; padding-right: 8px;"><i class="fas fa-chevron-down"></i></button>
			</div>
		<?php echo $vik->closeControl(); ?>
...
<script>
	...
	var CURR_SELECTED_WEEKLY_DAY = null;
	...
	jQuery(function($) {
		...
		<?php for ($i = 1, $dt = JFactory::getDate(); $i <= 7; $i++): ?>
			cloneButtons.push({
				text: Joomla.JText._('VAPMANAGEWDCLONEFROM').replace(/%s/, '<?php echo addslashes($dt->dayToString($i % 7)); ?>'),
				day: <?php echo $i; ?>,
				visible: function() {
					return this.day != CURR_SELECTED_WEEKLY_DAY && getWorkingTimesForWeeklyDay(this.day).length > 0;
				},
				action: function() {
					getWorkingTimesForWeeklyDay(this.day).forEach((time) => {
						delete time.id;
						vapAddTimeForm(time);
					});
				}
	...
	function vapClosedValueChanged(is) {
		...
		if (is) {
			jQuery('.inspector-repeatable').add('#wd-add-btn-group').hide();
		} else {
			jQuery('.inspector-repeatable').add('#wd-add-btn-group').show();
		}
	}
	...
	function fillWorkingDayForm(data) {
		...
		CURR_SELECTED_WEEKLY_DAY = wd.day;

		// disable clone buttons handle in case there are no working days yet
		jQuery('#wd-add-options').prop('disabled', !hasWorkingTimesForWeeklyDays(wd.day));
	}
	...
	function getWorkingTimesForWeeklyDay(day) {
		let wd = jQuery('#weekday-fieldset-' + day + ' input[name="wd_json[]"]').val();
		...
		// exclude the working days if closed
		return wd.filter(time => !time.closed);
	}

	function hasWorkingTimesForWeeklyDays(exclude) {
		for (let day = 1; day <= 7; day++) {
			if (day != exclude && getWorkingTimesForWeeklyDay(day).length) {
				return true;
			}
		}
