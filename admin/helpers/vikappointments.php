...
abstract class VikAppointmentsHelper
{
	...
	public static function printMenu()
	{
		...
		if ($auth['analytics']['numactives'] > 0)
		{
			...
			if ($auth['analytics']['actions']['analytics.options'])
			{
				$item = MenuFactory::createItem(JText::translate('VAPMENUOPTIONS'), $base_href . '&view=analytics&location=options', $task == 'analytics.options');
				$parent->addChild($item->setCustom('tags'));
			}
	...
	public static function load_css_js()
	{
		...
		$vik->addScript(VAPASSETS_URI . 'js/jquery-ui.draggable.min.js');
		...
		else if (VersionListener::isJoomla4x())
		{
			...
			if (VersionListener::isJoomla50())
			{
				...
				if (!VersionListener::isLowerThan('5.1'))
				{
					$vik->addStyleSheet(VAPASSETS_ADMIN_URI . 'css/adapter/J51.css');
				}
			}
		}
