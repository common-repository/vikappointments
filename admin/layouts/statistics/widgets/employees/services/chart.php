...
<div class="canvas-align-bottom employees-services-chart" id="employees-services-chart-<?php echo $widget->getID(); ?>">
	<canvas></canvas>
</div>
...
<script>
	 ...
	 * @var object
	 */
	if (typeof EMPLOYEES_SERVICES_CHARTS === 'undefined') {
		var EMPLOYEES_SERVICES_CHARTS = {};
	}
	...
	(function($) {
		...
		WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>] = (widget, data, config) => {
			...
			// retrieve selected employee name
			var employeeName = $('select[name="<?php echo $widget->getName() . '_' . $widget->getID(); ?>_employee"]')
				.find('option[value="' + config.employee + '"]')
					.text().trim();
			...
			const formatPointTitle = (items, data) => {
				if (employeeName) {
					// build title using the format [EMPLOYEE NAME] - [MONTH]
					return employeeName + ' - ' + items[0].label;
				}
			...
			const formatPointTooltip = (item, data) => {
				...
				// format as currency in case of earning
				if (config.valuetype == 'total') {
					label = VAPCurrency.getInstance().format(item.value);
				}
			...
			if (EMPLOYEES_SERVICES_CHARTS.hasOwnProperty(id) && EMPLOYEES_SERVICES_CHARTS[id].config.type != config.chart) {
				EMPLOYEES_SERVICES_CHARTS[id].destroy();
				delete EMPLOYEES_SERVICES_CHARTS[id];
			}
			...
			$.each(data, (lbl, services) => {
				...
				$.each(services, (id_service, service) => {
					...
					chartData.datasets[id_service].data.push(service[config.valuetype]);
				});
			});
			...
			if (!EMPLOYEES_SERVICES_CHARTS.hasOwnProperty(id)) {
				...
				EMPLOYEES_SERVICES_CHARTS[id] = new Chart(ctx, {
					type:    config.chart,
					data:    chartData,
					options: options,
				});
			...
			// otherwise update labels and values
				...
				// update chart data
				EMPLOYEES_SERVICES_CHARTS[id].data = chartData;

				// update format callbacks
				EMPLOYEES_SERVICES_CHARTS[id].options.scales.yAxes[0].ticks.callback = formatAxisY;
				EMPLOYEES_SERVICES_CHARTS[id].options.tooltips.callbacks.title = formatPointTitle;
				EMPLOYEES_SERVICES_CHARTS[id].options.tooltips.callbacks.label = formatPointTooltip;

				// refresh chart
				EMPLOYEES_SERVICES_CHARTS[id].update();
			}
		}
		...
		if (isset($data))
		{
			...
			$(function() {
				...
				WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>](
					...
					$('#employees-services-chart-<?php echo $widget->getID(); ?>'),
