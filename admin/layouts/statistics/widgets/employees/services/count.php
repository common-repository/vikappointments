...
<div class="canvas-align-center employees-services-count" id="employees-services-count-<?php echo $widget->getID(); ?>">
	<canvas></canvas>
</div>
...
<script>
	 ...
	 * @var object
	 */
	if (typeof EMPLOYEES_SERVICES_COUNT === 'undefined') {
		var EMPLOYEES_SERVICES_COUNT = {};
	}
		...
		WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>] = (widget, data, config) => {
			...
			$.each(data, (k, service) => {
				...
				if (isset($data))
				{
					...
					var sublabel = service.total;

					if (config.valuetype == 'total') {
						// format value as currency
						sublabel = VAPCurrency.getInstance().format(service.total);
					}
				...
				chartData.datasets[0].data.push(service.total);
				// hide highlight on hover
				chartData.datasets[0].hoverBorderColor.push('#0000');
				// push label
				chartData.labels.push(label);
				// fetch background color
				chartData.datasets[0].backgroundColor.push('#' + service.color);
			});
			...
			var options = {
				...
				tooltips: {
					...
					callbacks: {
						// include employee name within the tooltip title
						title: (items, data) => {
							if (employeeName) {
								return employeeName;
							}
						...
						label: (tooltipItem, data) => {
							...
							if (!isset($data))
							{
								...
								if (label) {
									...
									if (config.valuetype == 'total') {
										label += VAPCurrency.getInstance().format(value);
									} else {
			...
			if (!EMPLOYEES_SERVICES_COUNT.hasOwnProperty(id)) {
				...
				// update chart data
				EMPLOYEES_SERVICES_COUNT[id].data = chartData;

				// update chart options
				EMPLOYEES_SERVICES_COUNT[id].options = options;

				// refresh chart
				EMPLOYEES_SERVICES_COUNT[id].update();
			}
		...
		if (isset($data))
		{
			...
			$(function() {
				...
				WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>](
					...
					$('#employees-services-count-<?php echo $widget->getID(); ?>'),
