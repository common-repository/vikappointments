...
<div class="canvas-align-bottom packages-revenue-chart">
	<canvas></canvas>
</div>
...
<script>
	 ...
	 * @var object
	 */
	if (typeof PACKAGES_REVENUE_CHARTS === 'undefined') {
		var PACKAGES_REVENUE_CHARTS = {};
	}
	...
	(function($) {
		...
		WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>] = (widget, data, config) => {
			...
			if (PACKAGES_REVENUE_CHARTS.hasOwnProperty(id) && PACKAGES_REVENUE_CHARTS[id].config.type != config.chart) {
				PACKAGES_REVENUE_CHARTS[id].destroy();
				delete PACKAGES_REVENUE_CHARTS[id];
			}
			...
			if (!PACKAGES_REVENUE_CHARTS.hasOwnProperty(id)) {
				...
				PACKAGES_REVENUE_CHARTS[id] = new Chart(ctx, {
					type:    config.chart,
					data:    chartData,
					options: options,
				});
			...
			// otherwise update labels and values
				...
				// update chart data
				PACKAGES_REVENUE_CHARTS[id].data = chartData;

				// update format callbacks
				PACKAGES_REVENUE_CHARTS[id].options.scales.yAxes[0].ticks.callback = formatAxisY;
				PACKAGES_REVENUE_CHARTS[id].options.tooltips.callbacks.label = formatPointTooltip;

				// refresh chart
				PACKAGES_REVENUE_CHARTS[id].update();
			}
		}
