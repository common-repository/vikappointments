...
<div class="canvas-align-bottom options-revenue-chart">
	<canvas></canvas>
</div>
...
<script>
	 ...
	 * @var object
	 */
	if (typeof OPTIONS_REVENUE_CHARTS === 'undefined') {
		var OPTIONS_REVENUE_CHARTS = {};
	}
	...
	(function($) {
		...
		WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>] = (widget, data, config) => {
			...
			const formatPointTooltip = (item, data) => {
				...
				// extract option name from dataset
				let optName = data.datasets[item.datasetIndex].label;
				// create label in the form "OPTION: TOTAL"
				return ' ' + optName + ': ' + label;
			}
			...
			// iterate all dates
			$.each(data, (lbl, options) => {
				// iterate all options
				$.each(options, (optionName, value) => {
					// check whether we already created the dataset for this option
					if (!chartData.datasets.hasOwnProperty(optionName)) {
						// get progressive color
						let color = colorsPreset[colorIndex++ % colorsPreset.length];

						// nope, init data set
						chartData.datasets[optionName] = {
					...
					chartData.datasets[optionName].data.push(value);
				});
			});
			...
			if (!OPTIONS_REVENUE_CHARTS.hasOwnProperty(id)) {
				...
				OPTIONS_REVENUE_CHARTS[id] = new Chart(ctx, {
					type:    'line',
					data:    chartData,
					options: options,
				});
			...
			// otherwise update labels and values
				...
				// update chart data
				OPTIONS_REVENUE_CHARTS[id].data = chartData;

				// update format callbacks
				OPTIONS_REVENUE_CHARTS[id].options.scales.yAxes[0].ticks.callback = formatAxisY;
				OPTIONS_REVENUE_CHARTS[id].options.tooltips.callbacks.label = formatPointTooltip;

				// refresh chart
				OPTIONS_REVENUE_CHARTS[id].update();
			}
		}
