...
<div class="canvas-align-center options-revenue-count" id="options-revenue-count-<?php echo $widget->getID(); ?>">
	<canvas></canvas>
</div>
...
<script>
	 ...
	 * @var object
	 */
	if (typeof OPTIONS_REVENUE_COUNT === 'undefined') {
		var OPTIONS_REVENUE_COUNT = {};
	}
	...
	(function($) {
		...
		WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>] = (widget, data, config) => {
			...
			$.each(data, (optionName, count) => {
				var label = optionName;
				...
				if (isset($data))
				{
					...
					if (config.valuetype == 'total') {
						...
						sublabel = VAPCurrency.getInstance().format(count);
					}
			...
			if (!OPTIONS_REVENUE_COUNT.hasOwnProperty(id)) {
				...
				// update chart data
				OPTIONS_REVENUE_COUNT[id].data = chartData;

				// update chart options
				OPTIONS_REVENUE_COUNT[id].options = options;

				// refresh chart
				OPTIONS_REVENUE_COUNT[id].update();
			}
		...
		if (isset($data))
		{
			...
			$(function() {
				...
				WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>](
					...
					$('#options-revenue-count-<?php echo $widget->getID(); ?>'),
