...
<div class="canvas-align-bottom finance-payments-chart">
	<canvas></canvas>
</div>
...
<script>
	 ...
	 * @var object
	 */
	if (typeof FINANCE_PAYMENTS_CHARTS === 'undefined') {
		var FINANCE_PAYMENTS_CHARTS = {};
	}
	...
	(function($) {
		...
		WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>] = (widget, data, config) => {
			...
			const formatPointTooltip = (item, data) => {
				...
				return ' ' + payName + ': ' + VAPCurrency.getInstance().format(item.value);
			}
			...
			if (!FINANCE_PAYMENTS_CHARTS.hasOwnProperty(id)) {
				...
				FINANCE_PAYMENTS_CHARTS[id] = new Chart(ctx, {
					type:    'line',
					data:    chartData,
					options: options,
				});
			...
			// otherwise update labels and values
				...
				// update chart data
				FINANCE_PAYMENTS_CHARTS[id].data = chartData;

				// update format callbacks
				FINANCE_PAYMENTS_CHARTS[id].options.scales.yAxes[0].ticks.callback = formatAxisY;
				FINANCE_PAYMENTS_CHARTS[id].options.tooltips.callbacks.label = formatPointTooltip;

				// refresh chart
				FINANCE_PAYMENTS_CHARTS[id].update();
			}
		}
