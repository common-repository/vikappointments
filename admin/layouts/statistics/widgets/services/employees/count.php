...
<div class="canvas-align-center services-employees-count" id="services-employees-count-<?php echo $widget->getID(); ?>">
	<canvas></canvas>
</div>
...
<script>
	 ...
	 * @var object
	 */
	if (typeof SERVICES_EMPLOYEES_COUNT === 'undefined') {
		var SERVICES_EMPLOYEES_COUNT = {};
	}
		...
		WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>] = (widget, data, config) => {
			...
			$.each(data, (k, employee) => {
				...
				if (isset($data))
				{
					...
					var sublabel = employee.total;

					if (config.valuetype == 'total') {
						// format value as currency
						sublabel = VAPCurrency.getInstance().format(employee.total);
					}
				...
				chartData.datasets[0].data.push(employee.total);
				// hide highlight on hover
				chartData.datasets[0].hoverBorderColor.push('#0000');
				// push label
				chartData.labels.push(label);
				// fetch background color
				chartData.datasets[0].backgroundColor.push('#' + employee.color);
			});
			...
			var options = {
				...
				tooltips: {
					...
					callbacks: {
						// include service name within the tooltip title
						title: (items, data) => {
							if (serviceName) {
								return serviceName;
							}
			...
			if (!SERVICES_EMPLOYEES_COUNT.hasOwnProperty(id)) {
				...
				// update chart data
				SERVICES_EMPLOYEES_COUNT[id].data = chartData;

				// update chart options
				SERVICES_EMPLOYEES_COUNT[id].options = options;

				// refresh chart
				SERVICES_EMPLOYEES_COUNT[id].update();
			}
		...
		if (isset($data))
		{
			...
			$(function() {
				...
				WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>](
					...
					$('#services-employees-count-<?php echo $widget->getID(); ?>'),
