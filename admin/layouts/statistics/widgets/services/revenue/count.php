...
<div class="canvas-align-center services-revenue-count">
	<canvas></canvas>
</div>
...
<script>
	 ...
	 * @var object
	 */
	if (typeof SERVICES_REVENUE_COUNT === 'undefined') {
		var SERVICES_REVENUE_COUNT = {};
	}
	...
	(function($) {
		...
		WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>] = (widget, data, config) => {
			...
			if (!SERVICES_REVENUE_COUNT.hasOwnProperty(id)) {
				...
				// update chart data
				SERVICES_REVENUE_COUNT[id].data = chartData;

				// update chart options
				SERVICES_REVENUE_COUNT[id].options = options;

				// refresh chart
				SERVICES_REVENUE_COUNT[id].update();
			}
