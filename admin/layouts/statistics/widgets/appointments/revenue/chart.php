...
<div class="canvas-align-bottom appointments-revenue-chart">
	<canvas></canvas>
</div>
...
<script>
	 ...
	 * @var object
	 */
	if (typeof APPOINTMENTS_REVENUE_CHARTS === 'undefined') {
		var APPOINTMENTS_REVENUE_CHARTS = {};
	}
	...
	(function($) {
		...
		WIDGET_CALLBACKS[<?php echo $widget->getID(); ?>] = (widget, data, config) => {
			...
			if (APPOINTMENTS_REVENUE_CHARTS.hasOwnProperty(id) && APPOINTMENTS_REVENUE_CHARTS[id].config.type != config.chart) {
				APPOINTMENTS_REVENUE_CHARTS[id].destroy();
				delete APPOINTMENTS_REVENUE_CHARTS[id];
			}
			...
			if (!APPOINTMENTS_REVENUE_CHARTS.hasOwnProperty(id)) {
				...
				APPOINTMENTS_REVENUE_CHARTS[id] = new Chart(ctx, {
					type:    config.chart,
					data:    chartData,
					options: options,
				});
			...
			// otherwise update labels and values
				...
				// update chart data
				APPOINTMENTS_REVENUE_CHARTS[id].data = chartData;

				// update format callbacks
				APPOINTMENTS_REVENUE_CHARTS[id].options.scales.yAxes[0].ticks.callback = formatAxisY;
				APPOINTMENTS_REVENUE_CHARTS[id].options.tooltips.callbacks.label = formatPointTooltip;

				// refresh chart
				APPOINTMENTS_REVENUE_CHARTS[id].update();
			}
		}
