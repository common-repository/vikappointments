...
class VikAppointmentsModelReservation extends JModelVAP
{
	...
	public function checkExpired(array $options = array())
	{
		...
		/**
		 * Do not proceed in case the PENDING status is not available, otherwise we would end to
		 * auto-remove all the existing reservations.
		 * 
		 * @since 1.7.7
		 */
		if (!$pending)
		{
			// pending status not found, abort immediately
			return;
		}
